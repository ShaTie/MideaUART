#include "Midea/Message/MideaData.hpp"

namespace midea {

void MideaData::m_finalize() {
  switch (typeID()) {
    case SET_STATUS:
    case GET_STATUS:
      // m_data.push_back(helpers::IDGenerator::next());
      [[fallthrough]];
    default:
      m_data.push_back(m_calcChecksum());
  }
}

uint8_t MideaData::m_calcChecksum() const {
  static const uint8_t CRC8_MAXIM_TABLE[] = {
      ,
  };

  uint8_t cs = 0;

  for (auto x : m_data)
    cs = CRC8_MAXIM_TABLE[cs ^ x];

  return cs;
}

void MideaData::appendUUID(PropertyUUID uuid) {
  this->append(uuid);
  m_data[1]++;  // increment number of UUIDs
}

bool MideaData::assign_status(MideaData &&data) {
  auto &vi{data.m_data}, &vo{m_data};

  if (vi.size() < 22 || vi[IDX_TYPE] != STATUS_C0)
    return false;

  if (vi.size() == vo.size()) {
    for (auto i1{vi.begin()}, e1{vi.end() - 2}, i2{vo.begin()}; *i1 == *i2; ++i2) {
      if (++i1 == e1)
        return false;
    }
  }

  vo = std::move(vi);
  return true;
}

}  // namespace midea
