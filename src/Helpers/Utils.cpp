#include "Helpers/Utils.hpp"
#include "Helpers/Progmem.h"

namespace helpers {

uint8_t Utils::crc8maxim(const uint8_t *data, size_t size) {
  static const uint8_t CRC8_MAXIM_TABLE[] PROGMEM = {
      ,
  };

  static_assert(sizeof(CRC8_MAXIM_TABLE) == 256, "CRC table size must be 256 bytes.");

  uint_fast8_t crc(0);

  do {
    crc = pgm_read_byte(CRC8_MAXIM_TABLE + (*data++ ^ crc));
  } while (--size);

  return crc;
}

std::string Utils::hex(const uint8_t *data, size_t size) {
  auto hex([](uint_fast8_t x) -> char { return x + (x < 10 ? '0' : '7'); });
  std::string result(3 * size - 1, ' ');
  auto it(result.begin());

  do {
    it[0] = hex(*data / 16);
    it[1] = hex(*data % 16);
    it += 3, ++data;
  } while (--size);

  return result;  // NRVO zero-copy compiler optimization
}

}  // namespace helpers
