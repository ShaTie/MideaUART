...
class DataBodyControlB0 : public MideaData {
 public:
  ...
  DataBodyControlB0(uint8_t funcLo, uint8_t value, std::vector<uint8_t> values, uint8_t sound, uint8_t funcHi) {
    if (funcLo == 0x63 || funcLo == 0x64) {
      ...
      // Device: 0xAC, MessageTypeID: 0x02
      ...
      // Device: 0xAC, MessageTypeID: 0x02
      m_data = {0xB0, 0x02, funcLo, 0x02, static_cast<uint8_t>(values.size())};
      m_data.insert(m_data.end(), values.begin(), values.end());
      m_data.insert(m_data.end(), {0x1A, 0x00, 0x01, 0x01});
    } else if (funcHi == 0x00 &&
      ...
      // 0x0033: UUID_WIND_OFF_ME, 0x0039: UUID_SELF_CLEAN, 0x0042: UUID_BREEZE_AWAY
      ...
      // Control: Buzzer
      // Device: 0xAC, MessageTypeID: 0x02
      m_data = {0xB0, 0x01, 0x2C, 0x02, 0x01, value, sound};
    } else if (funcHi == 0x00 && funcLo == 0x5B) {
      // Device: 0xA1, MessageTypeID: 0x02
      m_data = {0xB0, 0x01, 0x5B, 0x00, 0x01, value, sound};
    } else if (funcHi == 0x00 && funcLo == 0x15) {
      // Query: Indoor humidity
      // Device: 0xAC, MessageTypeID: 0x03
      m_data = {0xB1, 0x01, 0x15, 0x00};
    } else if (funcHi == 17 && funcLo == 0x5B) {
      //
      // Device: 0xA1, MessageTypeID: 0x03
      m_data = {0xB1, 0x01, 0x5B, 0x00};
    } else if (funcHi == 18 && funcLo == 50) {
      // Query: "Wind ON me", "Wind OFF me"
      // Device: 0xAC, MessageTypeID: 0x03
      m_data = {0xB1, 0x02, 0x32, 0x00, 0x33, 0x00};
    } else if (funcLo == 101) {
      // Control function:
      // 0x004B: FRESH_AIR
      // Device: 0xAC, MessageTypeID: 0x02
      ...
      // Device: 0xAC, MessageTypeID: 0x02
      m_data = {0xB0, 0x02, 0x4B, 0x00, 0x04, 0x01, 0x03, 0xFF, 0xFF, 0x43, 0x00, 0x01, 0x03};
    }
    this->appendChecksum();
  }
};
